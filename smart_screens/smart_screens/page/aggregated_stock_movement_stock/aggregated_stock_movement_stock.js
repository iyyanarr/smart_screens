frappe.pages['aggregated-stock-movement-stock'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Aggregated Stock Movement Stock Only',
		single_column: true
	});

	// Initialize page
	new AggregatedStockMovementStock(page);
}

class AggregatedStockMovementStock {
	constructor(page) {
		this.page = page;
		this.make_form();
		this.add_filters();
	...
	validate_data() {
		frappe.call({
			method: 'smart_screens.smart_screens.page.aggregated_stock_movement_stock.aggregated_stock_movement_stock.validate_aggregation_accuracy',
			args: {
				filters: {
	...
	make_report() {
		...
		this.show_loading();
		frappe.call({
			method: 'smart_screens.smart_screens.page.aggregated_stock_movement_stock.aggregated_stock_movement_stock.get_aggregated_stock_data',
			args: {
				filters: filters
			},
			callback: (r) => {
					...
					<table class="table table-bordered sticky-table">
						<thead>
							<tr>
								...
								<th colspan="1" class="grand-total-header">Total</th>
	...
	show_common_code_details(common_code) {
		...
		frappe.call({
			method: 'smart_screens.smart_screens.page.aggregated_stock_movement_stock.aggregated_stock_movement_stock.get_common_code_details',
	...
	optimize_database() {
		frappe.confirm(
			...
			() => {
				...
				frappe.call({
					method: 'smart_screens.smart_screens.page.aggregated_stock_movement_stock.aggregated_stock_movement_stock.optimize_stock_ledger_indexes',
					callback: (r) => {
	...
	analyze_performance() {
		...
		frappe.call({
			method: 'smart_screens.smart_screens.page.aggregated_stock_movement_stock.aggregated_stock_movement_stock.analyze_query_performance',
			callback: (r) => {
				if (r.message && r.message.success) {
					frappe.msgprint({
	...
	benchmark_queries() {
		...
		frappe.call({
			method: 'smart_screens.smart_screens.page.aggregated_stock_movement_stock.aggregated_stock_movement_stock.benchmark_query_performance',
			args: {
				filters: {
					from_date: this.filters.from_date.get_value(),
					to_date: this.filters.to_date.get_value(),
