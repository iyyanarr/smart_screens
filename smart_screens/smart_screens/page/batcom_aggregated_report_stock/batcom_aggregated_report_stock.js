frappe.pages['batcom-aggregated-report-stock'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Batch & Compound Aggregated Stock Report',
		single_column: true
	});

	new BatComAggregatedStockReport(page);
}

class BatComAggregatedStockReport {
	constructor(page) {
		this.page = page;
		this.make_filters();
		this.add_buttons();
	...
	load_report() {
		...
		frappe.call({
			method: 'smart_screens.smart_screens.page.batcom_aggregated_report_stock.batcom_aggregated_report_stock.get_batcom_batch_balance_data',
	...
	bind_search_and_sort() {
		...
		$('#warehouse-filter').on('change', function () {
			...
			frappe.call({
				method: 'smart_screens.smart_screens.page.batcom_aggregated_report_stock.batcom_aggregated_report_stock.get_child_warehouses_api',
	...
	show_batch_details(common_code) {
		...
		frappe.call({
			method: 'smart_screens.smart_screens.page.batcom_aggregated_report_stock.batcom_aggregated_report_stock.get_batch_details_by_common_code',
			args: {
				common_code: common_code,
