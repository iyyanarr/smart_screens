"""
Batch & Compound Aggregated Stock Report (No Valuation)
...
report_name="BatCom Aggregated Stock Report",
...
user=frappe.session.user
		...
		t3 = time.time()
		all_data, excluded_count = exclude_batches(all_data, "BatCom Aggregated Stock Report")
		t4 = time.time()
...
user=frappe.session.user
		...
		frappe.publish_realtime(
'batcom_aggregated_progress',
{'step': 4, 'total': 4, 'message': f'Completed! Generated {len(aggregated_data["data"])} aggregated Stock items', 'percent': 100},
user=frappe.session.user
)
		...
		frappe.log_error(
f"Error in BatCom Aggregated Stock Report: {str(e)}\n{frappe.get_traceback()}",
"BatCom Aggregated Stock Report Error"
)
		return {
			"success": False,
			"error": str(e),
...
def get_batch_details_by_common_code(common_code, filters=None, warehouse=''):
	...
	try:
		...
		# Exclude batches
		all_batches_data, excluded_count = exclude_batches(all_batches_data, "BatCom Aggregated Stock Report")
...
def export_to_excel(filters=None, warehouse=''):
	...
	try:
		...
		return export_aggregated_data_to_excel(
			...
			report_title="BatCom Aggregated Stock Report",
			filters=filters,
			warehouse_filter=warehouse,
