frappe.pages['rm-aggregated-report'].on_page_load = function (wrapper) {
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'Raw Material Aggregated Report',
        single_column: true
    });

    new RMAggregatedReport(page);
}

class RMAggregatedReport {
    constructor(page) {
        this.page = page;
        this.make_filters();
        this.add_buttons();
    ...
    load_report() {
        ...
        frappe.call({
            method: 'smart_screens.smart_screens.page.rm_aggregated_report.rm_aggregated_report.get_rm_batch_balance_data',
					...
					<table class="table table-bordered aggregated-table" id="rm-data-table">
						<thead>
							<tr>
								...
								<th class="sortable-header" data-column="last_purchase_rate">Last Rate <i class="fa fa-sort"></i></th>
								<th class="sortable-header" data-column="balance_value">Valuation <i class="fa fa-sort"></i></th>
    ...
    render_table_rows(data) {
        ...
        data.forEach(row => {
            const rowHtml = `
				<tr data-item-code="${row.item_code}">
					...
					<td class="qty-cell text-right text-muted">${this.format_currency(row.last_purchase_rate)}</td>
					<td class="qty-cell valuation-cell"><strong>${this.format_currency(row.balance_value)}</strong></td>
        ...
        const grandTotalHtml = `
			<tr class="grand-total-row">
				...
				<td></td>
				<td class="qty-cell valuation-cell"><strong>${this.format_currency(this.grand_total.balance_value)}</strong></td>
				<td></td>
			</tr>`;
    ...
    bind_search_and_sort() {
        ...
        $('#warehouse-filter').on('change', function () {
            ...
            frappe.call({
                method: 'smart_screens.smart_screens.page.rm_aggregated_report.rm_aggregated_report.get_child_warehouses_api',
                ...
                callback: (r) => {
                    if (r.message) {
                        ...
                        const aggregated = {};
                        const grand_total = { opening_qty: 0, in_qty: 0, out_qty: 0, balance_qty: 0, balance_value: 0 };

                        // Store purchase rates from original report data for reference if missing in raw data
                        const itemRates = {};
                        me.report_data.forEach(d => { itemRates[d.item_code] = d.last_purchase_rate; });
                        ...
                        filtered_raw.forEach(row => {
                            ...
                            if (!aggregated[key]) {
                                aggregated[key] = {
                                    ...
                                    last_purchase_rate: itemRates[key] || 0
                                };
                            }
                            ...
                            // Recalculate valuation based on last_purchase_rate
                            const valuation = bal * (aggregated[key].last_purchase_rate || 0);
                            aggregated[key].balance_value += valuation;
                            ...
                            grand_total.balance_value += valuation;
                        });
    ...
    show_batch_details(item_code) {
        frappe.call({
            method: 'smart_screens.smart_screens.page.rm_aggregated_report.rm_aggregated_report.get_batch_details_by_item_code',
				...
				<tbody>
					${data.batches.map(b => `
						<tr>
							...
							<td class="text-right">${me.format_currency(b.balance_value)}</td>
						</tr>
					`).join('')}
				</tbody>
			</table>
    ...
    export_to_excel() {
        ...
        frappe.call({
            method: 'smart_screens.smart_screens.page.rm_aggregated_report.rm_aggregated_report.export_to_excel',
            args: {
                filters: filters,
