frappe.pages['spp-aggregated-report-stock'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'SPP Aggregated Stock Report',
		single_column: true
	});

	new SPPAggregatedStockReport(page);
}

class SPPAggregatedStockReport {
	constructor(page) {
		this.page = page;
		this.make_filters();
		this.add_buttons();
	...
	load_report() {
		...
		frappe.call({
			method: 'smart_screens.smart_screens.page.spp_aggregated_report_stock.spp_aggregated_report_stock.get_spp_batch_balance_data',
	...
	render_table_rows(data) {
		...
		data.forEach(row => {
			const rowHtml = `
1: 				<tr data-common-code="${row.common_code}">
...
`;
	...
	bind_search_and_sort() {
		...
		$('#warehouse-filter').on('change', function () {
			...
			frappe.call({
				method: 'smart_screens.smart_screens.page.spp_aggregated_report_stock.spp_aggregated_report_stock.get_child_warehouses',
	...
	show_batch_details(common_code) {
		...
		frappe.call({
			method: 'smart_screens.smart_screens.page.spp_aggregated_report_stock.spp_aggregated_report_stock.get_batch_details_by_common_code',
			args: {
				common_code: common_code,
