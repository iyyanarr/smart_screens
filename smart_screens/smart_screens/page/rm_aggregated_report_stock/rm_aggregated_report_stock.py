"""
Raw Material Aggregated Stock Report (No Valuation)
...
def get_rm_batch_balance_data(filters=None):
	...
	try:
		...
		all_data = fetch_batch_balance_data(
			report_name="Raw Material Aggregated Stock Report",
		...
		t3 = time.time()
		all_data, excluded_count = exclude_batches(all_data, "Raw Material Aggregated Stock Report")
		t4 = time.time()
		...
		frappe.publish_realtime(
			'rm_aggregated_progress',
			{'step': 4, 'total': 4, 'message': f'Completed! Generated {len(aggregated_data["data"])} RM Stock items', 'percent': 100},
			user=frappe.session.user
		)
...
def aggregate_by_item_code_rm(data):
	"""Custom aggregation for RM report (Stock Only - No Valuation)"""
	import pandas as pd
	...
	qty_columns = ['opening_qty', 'in_qty', 'out_qty', 'balance_qty']
...
def get_batch_details_by_item_code(item_code, filters=None, warehouse=''):
	...
	try:
		...
		# Exclude batches
		all_batches_data, excluded_count = exclude_batches(all_batches_data, "Raw Material Aggregated Stock Report")
	...
	try:
		...
		if data:
			...
			return export_rm_data_to_excel(
				data=data,
				grand_total=grand_total,
				report_title="Raw Material Aggregated Stock Report",
				filters=filters,
				warehouse_filter=warehouse
			)
